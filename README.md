...
Поместите ваши файлы с историческими данными в формате `.txt` или `.csv` в папку `data-connector/Data/Tinkoff/`.

Шаблон по умолчанию настроен на чтение текстовых файлов с разделителем-табуляцией и форматом даты `ДД.ММ.ГГГГ ЧЧ:ММ`. Вы можете легко адаптировать его под свой формат, изменив файл `utils/custom_csv.py`.
